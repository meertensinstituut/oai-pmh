    ...
    <html>
      ...
      <body>
        ...
        <p class="intro">You are viewing an HTML version of the XML OAI response. To see the
          underlying XML use your web browsers view source option. More information about this XSLT
          is at the <a href="#moreinfo">bottom of the page</a>.</p>
        ...
        <p>An XSLT file has converted the <a href="http://www.openarchives.org">OAI-PMH 2.0</a>
          responses into XHTML which looks nice in a browser which supports XSLT such as Mozilla,
          Firebird and Internet Explorer. The XSLT file was created by <a
            href="http://www.ecs.soton.ac.uk/people/cjg">Christopher Gutteridge</a> at the
          University of Southampton as part of the <a href="http://software.eprints.org">GNU EPrints
            system</a>, and is freely redistributable under the <a href="http://www.gnu.org"
          >GPL</a>.</p>
        <p>If you want to use the XSL file on your own OAI interface you may but due to the way XSLT
          works you must install the XSL file on the same server as the OAI script, you can't just
          link to this copy.</p>
...
-->
    ...
    <xsl:choose>
      <xsl:when test="$identifier">
        <p>This is a list of metadata formats available for the record "<xsl:value-of
            ...
            select="oai:metadataFormat/oai:metadataPrefix"/></p>
      </xsl:when>
      <xsl:otherwise>
    ...
    <table class="values">
      <tr>
        ...
        <td class="value">
          ...
          <xsl:text> </xsl:text>
          <a class="link"
            href="?verb=GetRecord&amp;metadataPrefix=oai_dc&amp;identifier={oai:identifier}"
            >oai_dc</a>
          <xsl:text> </xsl:text>
  ...
  <xsl:template match="oai:setSpec">
    <tr>
      ...
      <td class="value">
        <xsl:value-of select="."/>
        <xsl:text> </xsl:text>
        <a class="link" href="?verb=ListIdentifiers&amp;metadataPrefix=oai_dc&amp;set={.}"
          >Identifiers</a>
        <xsl:text> </xsl:text>
    ...
    <table class="values">
      <tr>
        ...
        <td class="value">
          <xsl:value-of select="."/>
          <xsl:text> </xsl:text>
          <a class="link" href="?verb={/oai:OAI-PMH/oai:request/@verb}&amp;resumptionToken={.}"
            >Resume</a>
        </td>
      </tr>
    </table>
  ...
  <xsl:template match="node()" mode="xmlMarkup">
    <div class="xmlBlock"> &lt;<span class="xmlTagName"><xsl:value-of select="name(.)"
        /></span><xsl:apply-templates select="@*" mode="xmlMarkup"/>&gt;<xsl:apply-templates
        select="node()" mode="xmlMarkup"/>&lt;/<span class="xmlTagName"><xsl:value-of
          select="name(.)"/></span>&gt; </div>
  </xsl:template>
  ...
  <xsl:template match="@*" mode="xmlMarkup">
    <xsl:text> </xsl:text><span class="xmlAttrName"><xsl:value-of select="name()"/></span>="<span
      class="xmlAttrValue"><xsl:value-of select="."/></span>" </xsl:template>
